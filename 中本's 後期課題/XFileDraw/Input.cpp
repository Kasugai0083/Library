...
bool InitInput(HINSTANCE hInstance, HWND hWindow)
{
	...
	if (FAILED(hr))
	{
		MessageBox(hWindow, (char*)"error", (char*)"errorA", MB_OK);
		return false;
	}
	...
	if (FAILED(hr))
	{
		MessageBox(hWindow, (char*)"error", (char*)"errorB", MB_OK);
		return false;
	}
	...
	if (FAILED(hr))
	{
		MessageBox(hWindow, (char*)"error", (char*)"errorC", MB_OK);
		return false;
	}

	hr = g_KeyDevice->SetCooperativeLevel(hWindow, DISCL_BACKGROUND | DISCL_NONEXCLUSIVE);

	if (FAILED(hr))
	{
		MessageBox(hWindow, (char*)"error", (char*)"errorD", MB_OK);
		return false;
	}
	...
	if (FAILED(g_MouseDevice->SetCooperativeLevel(
		FindWindowA(TEXT("XFileDraw"), nullptr),
